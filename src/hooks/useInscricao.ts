...
import type { Candidato, DadosComplementares, DadosPessoais, Edital, Inscricao, ProjetoPesquisa, AppMessage } from '../types'
...
export function useInscricao(edital: Edital | null) {
	...
	const [inscricaoHistorico, setInscricaoHistorico] = useState<Inscricao | null>(null)
	const [dadosPessoais, setDadosPessoais] = useState<DadosPessoais>(getInitialDadosPessoais)
	const [dadosComplementares, setDadosComplementares] =
		useState<DadosComplementares>(getInitialDadosComplementares)
	const [projetoPesquisa, setProjetoPesquisa] = useState<ProjetoPesquisa>(getInitialProjetoPesquisa)
	const [saving, setSaving] = useState(false)
	const [inscricaoEnviada, setInscricaoEnviada] = useState(false)
	const [message, setMessage] = useState<AppMessage | null>(null)

	const showMessage = (text: string, severity: AppMessage['severity'] = 'success') => {
		setMessage({ text, severity })
	}

	const carregarDadosExistentes = useCallback((insc: Inscricao, cand: Candidato | null) => {
		if (cand) {
	...
	const salvarEtapa2 = useCallback(async () => {
		...
		try {
			...
			if (payload.update) {
				...
				inscricaoAtualizada = await inscricaoService.create(payload.dados as Partial<Inscricao>)
			}
