...
export default function InscricaoStepper() {
	...
	useEffect(() => {
		editalService
			...
			.then(async (e) => {
				setEdital(e)
				const editalComDocs = await editalService.findWithDocumentos(e.id)
				setTiposDocumento(editalComDocs.tiposDocumento ?? [])
			})
			.catch(async () => {
				setSemEditalAberto(true)
				const proximo = await editalService.findProximo().catch(() => null)
	...
	if (semEditalAberto) {
		return (
			...
			>
				...
				>
					<Chip label="Em breve" color="primary" sx={{ mb: 3, fontWeight: 600 }} />

					<Typography variant="h4" sx={{ fontWeight: 700 }} color="primary.main" gutterBottom>
						Site em construção
					</Typography>

					<Typography variant="body1" color="text.secondary" sx={{ mb: 3 }}>
						Estamos preparando a plataforma de inscrições do Processo Seletivo PPGPCA da UFFS.
						Em breve você poderá realizar sua inscrição por aqui.
					</Typography>
					...
					{editalProximo && (
						<>
							...
							<Typography
								...
								sx={{ display: 'block', letterSpacing: 2, mb: 1 }}
							>
			...
			<Box sx={{ width: '100%', textAlign: 'center' }}>
				<Paper sx={{ p: 4 }}>
					...
					<Typography variant="body1" color="text.secondary" sx={{ mb: 2 }}>
						Sua inscrição foi registrada com o número <strong>{inscricao.id}</strong>.
					</Typography>
					<Alert severity="info" sx={{ textAlign: 'left', mb: 3 }}>
						...
						<Link
							...
							sx={{ fontWeight: 700, color: '#f5c842', '&:hover': { color: '#ffe680' } }}
						>
					...
					<Box sx={{ display: 'flex', justifyContent: 'flex-end' }}>
						<Button variant="outlined" color="primary" size="small" onClick={reiniciar}>
							Voltar ao início
						</Button>
					</Box>
				</Paper>
	...
	return (
		<Box sx={{ width: '100%' }}>
			...
			<Paper sx={{ p: 3 }}>
				...
				{activeStep === 1 && (
					<EtapaProjetoPesquisa dados={projetoPesquisa} onChange={setProjetoPesquisa} />
				)}
				...
				{activeStep === 3 && (
					<EtapaFormacao dados={dadosComplementares} onChange={setDadosComplementares} />
				)}
