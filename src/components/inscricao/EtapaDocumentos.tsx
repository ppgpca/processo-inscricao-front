...
export default function EtapaDocumentos({
	...
	const documentoAtual = (idTipo: number) => obterDocumentoAtual(documentos, idTipo)
	...
	return (
		<Box>
			...
			<List disablePadding>
				{ordenarTiposDocumentoAtivos(tiposDocumento).map((tipo, idx) => {
					const doc = documentoAtual(tipo.id)
					const isUploading = uploading === tipo.id

					return (
						<Paper key={tipo.id} variant="outlined" sx={{ mb: 1.5 }}>
							<ListItem
								sx={{ py: 1.5 }}
								secondaryAction={
