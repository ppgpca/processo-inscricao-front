...
interface EtapaDocumentosProps {
	...
	onDocumentosAtualizados?: (docs: Documento[]) => void;
}

export default function EtapaDocumentos({
	idInscricao,
	tiposDocumento,
	onDocumentosAtualizados,
	...
	const documentoAtual = (idTipo: number) =>
		obterDocumentoAtual(documentos, idTipo);

	const baixarDocumento = useCallback(
		async (idTipo: number, nomeOriginal: string) => {
			try {
				const url = documentoService.getDownloadUrl(
					idInscricao,
					idTipo,
				);
				...
				const nomeArquivo = extrairNomeArquivoDownload(
					disposition,
					nomeOriginal,
				);
	...
	const handleUpload = async (idTipo: number, file: File) => {
		...
		try {
			...
			setUploading(null);
			const input = inputRefs.current[idTipo];
			if (input) {
				input.value = "";
			}
		}
	...
	const handleRemover = async (idTipo: number) => {
		try {
	...
	const allObrigatoriosEnviados = todosObrigatoriosEnviados(
		tiposDocumento,
		documentos,
	);
	...
	return (
		<Box>
			...
			<Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
				Faça o upload dos documentos exigidos. Documentos obrigatórios
				estão marcados com *. Formatos aceitos: PDF, JPG, PNG (máx. 10
				MB).
			</Typography>
			...
			{allObrigatoriosEnviados && (
				...
				>
					Todos os documentos obrigatórios foram enviados. Você pode
					finalizar a inscrição.
				</Alert>
			)}

			<List disablePadding>
				{ordenarTiposDocumentoAtivos(tiposDocumento).map(
					(tipo, idx) => {
							...
							>
								...
								<Box sx={{ px: 2, py: 1 }}>
									<input
										...
										style={{ display: "none" }}
										ref={(el) => {
											inputRefs.current[tipo.id] = el;
										}}
										onChange={(e) => {
											const file = e.target.files?.[0];
											if (file)
												handleUpload(tipo.id, file);
										}}
									/>
									<Box
										component="label"
